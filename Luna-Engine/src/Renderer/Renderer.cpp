	...
	Shader* shader, Shader* depthMapShader)
{
	...
	light.object->m_Light.FrameSetup(lightManager, camera.object, camera.objectTransform, light.objectTransform, shader);
	...
	SetShaderFrame(camera, light, depthMapShader, shader);
	for (auto& [id, meshComponent] : meshComponents)
	{
		auto transformIt = transforms.find(meshComponent->gameObject);
		if (transformIt != transforms.end())
...
void Renderer::SetShaderFrame(ObjectTransformPairing<Camera>& camera, ObjectTransformPairing<LightComponent>& light, Shader* depthmapShader, Shader* shader)
{
