...
void Light::FrameSetup(LightManager* lightManager, Camera* camera, Transform* cameraTransform, Transform* lightTransform, Shader* shader, FrameBuffer* framebuffer)
{
    glEnable(GL_DEPTH_TEST);

    std::vector<glm::mat4> matrices = lightManager->GenerateLightSpaceMatrices(camera, cameraTransform, framebuffer, lightTransform->Forward());
    glBindBuffer(GL_UNIFORM_BUFFER, m_MatricesUBO);
